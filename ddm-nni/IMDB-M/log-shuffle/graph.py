...
def pretrain(model, train_loader, optimizer, device, epoch, logger, loss_record):
    model.train()
    loss_list = []
    ...
    loss_record.append({'epoch': epoch, 'loss': np.mean(loss_list)}) 
...
def main(cfg):
    ...
    eval_loader = GraphDataLoader(graphs, collate_fn=collate_fn, batch_size=len(graphs) // 3, shuffle=True)
    ...
    for i, seed in enumerate(cfg.seeds):
        ...
        for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
            ...
            pretrain(model, train_loader, optimizer, cfg.DEVICE, epoch, logger, loss_record)
...
if __name__ == "__main__":
    ...
    output_dir = osp.join(root_dir, 'log-shuffle')
