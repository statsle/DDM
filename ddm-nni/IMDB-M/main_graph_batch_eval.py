...
def shuffle(graphs,r):
    """Args:
    graphs: input graph dataset, the graph class must be 2
    r:the positive(label=1) ratio in the first half of dataset
    """
    g=[]
    ...
    n_2=len(graphs)
    n=int(n_2/2)
    label_0=torch.where((labels==0))[0]
    label_1=torch.where((labels==1))[0]
    g_1=[]
    g_2=[]
    for j in range(int(n*r)):
        g_1.append(graphs[label_1[j]])
        g_2.append(graphs[label_0[j]])
    for j in range(int(n*r),n):
        g_1.append(graphs[label_0[j]])
        g_2.append(graphs[label_1[j]])
    g_1.extend(g_2)
    return g_1
...
def main(cfg):
    ...
    graphs, (num_features, num_classes) = load_graph_classification_dataset(cfg.DATA.data_name,
                                                                            ...
                                                                            PE=False)
    # graphs = shuffle(graphs, cfg.eval_ratio)
    ...
    # eval_loader = GraphDataLoader(graphs, collate_fn=collate_fn, batch_size=500, shuffle=False)

    eval_loader = []
    eval_loader.append(collate_fn(graphs[:500]))
    eval_loader.append(collate_fn(graphs[500: 1000]))
    eval_loader.append(collate_fn(graphs[1000:]))
...
if __name__ == "__main__":
    ...
    output_dir = osp.join(root_dir, 'log-eval-ratio0.2-sigmod')
