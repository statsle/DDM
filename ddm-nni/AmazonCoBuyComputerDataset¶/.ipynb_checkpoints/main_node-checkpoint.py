...
def pretrain(model, graph, feat, optimizer, epoch, logger=None):
...
def main(cfg):
    ...
    for i, seed in enumerate(cfg.seeds):
        ...
        for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
            ...
            pretrain(model, graph, feat, optimizer, epoch, logger=logger)
            if ((epoch + 1) % 1 == 0) & (epoch > 50):
                model.eval()
                ...
                logger.info(f"Epoch {epoch}: get test acc score: {acc: .5f}")
                logger.info(f"best_f1 {best_acc:.5} at epoch {best_acc_epoch}")
    ...
    logger.info((f"# final_acc: {final_acc:.5f}±{final_acc_std:.5f}"))
    return final_acc
